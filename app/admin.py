...
def _parse_tid(value) -> int:
    """Parse Telegram ID safely. Allows /app without tid and avoids FastAPI int_parsing errors."""
    if value is None:
        return 0
...
def _validate_image_file(path: str) -> None:
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="الصورة غير واضحة أو ليست صورة صحيحة. يرجى الالتقاط من الكاميرا بإضاءة واضحة.",
        )
...
@app.get("/health")
def health():
    return {"ok": True, "brand": settings.brand_name}
...
def dashboard(request: Request, admin=Depends(auth)):
    return templates.TemplateResponse(
        "dashboard.html",
        {
            ...
            "prices": db.prices(),
            "methods": db.payment_methods(active=False),
        },
    )


@app.get("/app", response_class=HTMLResponse)
def mini_app(request: Request, tid: Optional[str] = None):
    telegram_id = _parse_tid(tid)
    u = db.user(telegram_id) if telegram_id else None
    return templates.TemplateResponse(
        "mini_app.html",
        {
            "request": request,
            ...
            "tid": telegram_id,
            "prices": db.prices(),
            "methods": db.payment_methods(active=True),
            "deposits": db.my_deposits(telegram_id) if telegram_id else [],
            "withdraws": db.my_withdraws(telegram_id) if telegram_id else [],
            "txs": db.my_transactions(telegram_id) if telegram_id else [],
            "notes": db.notifications(telegram_id) if telegram_id else [],
            "refs": db.referrals(telegram_id) if telegram_id else [],
...
def kyc_app(request: Request, tid: Optional[str] = None):
    telegram_id = _parse_tid(tid)
    return templates.TemplateResponse(
        "kyc_webapp.html",
        {"request": request, "brand": settings.brand_name, "tid": telegram_id},
    )


@app.post("/kyc-app/submit", response_class=HTMLResponse)
async def kyc_app_submit(
    request: Request,
    ...
    _validate_image_file(id_path)
    _validate_image_file(selfie_path)
    ...
    return templates.TemplateResponse(
        "kyc_success.html",
        {"request": request, "brand": settings.brand_name, "request_no": no},
    )


@app.post("/app/deposit")
async def app_deposit(
    tid: int = Form(...),
    amount: float = Form(...),
    currency: str = Form(...),
    platform: str = Form(""),
...
async def app_withdraw(
    tid: int = Form(...),
    amount: float = Form(...),
    currency: str = Form(...),
    local_receiver: str = Form(...),
    usdt_txid: str = Form(""),
    proof: UploadFile = File(...),
):
    _validate_common_order(amount, currency)
...
def statement(tid: Optional[str] = None):
    telegram_id = _parse_tid(tid)
    u = db.user(telegram_id) if telegram_id else None
    txs = db.my_transactions(telegram_id) if telegram_id else []
    lines = [
        "TrustPay Statement",
        f"User: {u['full_name'] if u else telegram_id}",
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=statement_{telegram_id}.pdf"},
    )
...
def prices_update(
    buy_usdt: float = Form(...),
    sell_usdt: float = Form(...),
    fixed_fee: float = Form(...),
    percent_fee: float = Form(...),
    admin=Depends(auth),
):
